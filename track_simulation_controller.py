...
pause_event = threading.Event() # 新增：用于暂停
...
class TrackSimulatorApp:
    def __init__(self, root):
        ...
        self.root.geometry("550x640") # 窗口大小
        ...
        ttk.Checkbutton(path_frame, text="跳过 23 秒启动等待", variable=self.skip_wait_var).grid(row=2, column=0, sticky="w", padx=180, pady=5)
        ...
        self.pace_minkm = tk.StringVar(value="6.0") # 默认6分钟/公里
    ...
    def start_simulation(self):
        try:
            ...
            pause_event.set() # .set() 意味着 "未暂停" (允许 wait() 通过)
            ...
            self.pause_button.config(text="暂停", state=tk.NORMAL) # 启用暂停
            self.stop_button.config(state=tk.NORMAL) # 启用停止
            
            self.simulation_thread = threading.Thread(
                target=run_simulation_thread,
                args=(self.status_queue, ld_folder, emu_index, full_path_points, delay_seconds, skip_wait), # 传入 skip_wait
                daemon=True
            )
    ...
    def toggle_pause(self):
        ...
        if pause_event.is_set():
            # 正在运行 -> 暂停
            pause_event.clear() # 命令线程在 wait() 处阻塞
            self.pause_button.config(text="继续")
            self.status_label.config(text="模拟已暂停。", foreground="orange")
        else:
            # 已暂停 -> 继续
            pause_event.set() # 命令线程从 wait() 处释放
    ...
    def stop_simulation(self):
        ...
        stop_simulation_event.set() # 1. 发送停止信号
        pause_event.set() # 2. 确保线程没有卡在 pause_event.wait()
        # GUI 状态将由 check_queue 中的 "DONE" 或 "STATUS" 消息重置
